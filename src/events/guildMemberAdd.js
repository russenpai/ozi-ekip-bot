...
<a:tik:922133110135615488> Sunucumuza **${member.guild.name}** üyesinin davetiyle katıldın ve seninle birlikte ailemiz ${üyesayısı} kişi oldu!\n
<a:tik:922133110135615488> Sunucu kurallarımız ${kurallar} kanalında belirtilmiştir. Unutma sunucu içerisinde ki \`ceza-i işlemler\` kuralları okuduğunu varsayarak gerçekleştirilecek.\n
...
kayitchannel.wsend(`
${member.guild.name}'e Hoş geldin ${member} biz de seni bekliyorduk, hesabın __${memberGün} ${memberAylar} ${memberTarih}__ tarihinde \`` + moment(member.user.createdTimestamp).fromNow() + `\` oluşturulmuş ${guvenilirlik ? `Şüpheli! ${red}` : `Güvenli! ${green}` }\n
<a:tik:922133110135615488> Sunucumuza **${invite.inviter}** üyesinin davetiyle katıldın ve seninle birlikte ailemiz ${üyesayısı} kişi oldu!\n
<a:tik:922133110135615488> Sunucu kurallarımız ${kurallar} kanalında belirtilmiştir. Unutma sunucu içerisinde ki \`ceza-i işlemler\` kuralları okuduğunu varsayarak gerçekleştirilecek.\n
Tagımıza ulaşmak için herhangi bir kanala \`.tag\` yazabilirsiniz. ${tagModedata ? tagModedata.tagMode === true ? `(**Şuan da taglı alımdayız**)`:``:``} :tada: :tada: :tada:`);
channel.wsend(`${member}, ${invite.inviter.tag} davetiyle katıldı! (**${total}**)`)
}
