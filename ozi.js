...
client.on("userUpdate", async function(eskiii, yeniii) {
  const guildID = "830565831845871627"
  const roleID = "855159733048311818"//taglı_rol
  const tag = "Shéw"
  const log2 = '915754150455828480'
  ...
  if (yeniii.username !== eskiii.username) {
      ...
      if (eskiii.username.includes(tag) && !yeniii.username.includes(tag)) {
        if (yeniii.discriminator === "1983") return client.channels.cache.get(log2).send(`${yeniii} İsim tagımızı bıraktı ama hala üzerinde etiket olduğu için \`Tagges\` rolünü almadım`);
 ...
 if (yeniii.discriminator !== eskiii.discriminator) {
      if (eskiii.discriminator == "1983" && yeniii.discriminator !== "1983") {
      ...
      } else if (eskiii.discriminator !== "1983" && yeniii.discriminator == "1983") {
          member.roles.add(roleID)
...
client.on("guildMemberAdd", member => {
  ...
  if (member.user.discriminator === "1983") {
    setTimeout(function(){  
   client.channels.cache.get(log2).send(`${member} sunucuya \`1983\` etiketiyle geldiği için \`tagges\` rolünü verdim.`)
  }, 2000);
  setTimeout(function(){  
   member.roles.add(role)
  }, 5000);
